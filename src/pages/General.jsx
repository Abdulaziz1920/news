...
const Entertainment = () => {
  useEffect(() => {
    document.querySelector("head title").textContent = "MEGA NEWS | General";
  });
    ...
    const getData = async () => {
      try {
        let { data } = await axios(
          `${ENDPOINT}category=general&q=${search}&pageSize=${PAGE_LIMIT}&page=${page}&apiKey=${APIKEY}`
        );
        let { data: totalData } = await axios(
          `${ENDPOINT}category=general&q=${search}&apiKey=${APIKEY}`
        );
