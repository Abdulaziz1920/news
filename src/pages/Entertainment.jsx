    ...
    const getData = async () => {
      try {
        let { data } = await axios(
          `${ENDPOINT}category=entertainment&q=${search}&pageSize=${PAGE_LIMIT}&page=${page}&apiKey=${APIKEY}`
        );
        let { data: totalData } = await axios(
          `${ENDPOINT}category=entertainment&q=${search}&apiKey=${APIKEY}`
        );
