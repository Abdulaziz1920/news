...
const Entertainment = () => {
  useEffect(() => {
    document.querySelector("head title").textContent = "MEGA NEWS | Technology";
  });
    ...
    const getData = async () => {
      try {
        let { data } = await axios(
          `${ENDPOINT}category=technology&q=${search}&pageSize=${PAGE_LIMIT}&page=${page}&apiKey=${APIKEY}`
        );
        let { data: totalData } = await axios(
          `${ENDPOINT}category=technology&q=${search}&apiKey=${APIKEY}`
        );
