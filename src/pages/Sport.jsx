...
const Entertainment = () => {
  useEffect(() => {
    document.querySelector("head title").textContent = "MEGA NEWS | Sports";
  });
    ...
    const getData = async () => {
      try {
        let { data } = await axios(
          `${ENDPOINT}category=sport&q=${search}&pageSize=${PAGE_LIMIT}&page=${page}&apiKey=${APIKEY}`
        );
        let { data: totalData } = await axios(
          `${ENDPOINT}category=sport&q=${search}&apiKey=${APIKEY}`
        );
